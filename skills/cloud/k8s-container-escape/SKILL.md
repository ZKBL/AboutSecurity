...
- ⛔**必读** 容器逃逸详细手法（挂载逃逸/内核漏洞/特权容器/cgroup）→ `read_skill(id="k8s-container-escape", path="references/escape-techniques.md")`
- ⛔**必读** 集群层面攻击（API Server/etcd/RBAC/横向移动）→ `read_skill(id="k8s-container-escape", path="references/cluster-attacks.md")`
...
→ 完整逃逸手法 → `read_skill(id="k8s-container-escape", path="references/escape-techniques.md")`
...
→ 详细集群攻击 → `read_skill(id="k8s-container-escape", path="references/cluster-attacks.md")`
